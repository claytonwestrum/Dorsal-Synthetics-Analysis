function dorsalResults = createDorsalResults2(DataType, varargin)
...
% minEmbryos = 1; %minimum number of embryos per bin


%options must be specified as name, value pairs. unpr edictable errors will
%occur, otherwise.
...
for nc = 1
    ...
    filteredWeightedSE = @(y) nanstd(bootstrp(nSamples, @(x) filteredWeightedMean(x), y,...
        'Weights',nSchnitzBinTotalWithZeros{nc}),0, binDim)';
