function [results,chain,s2chain]  = fitstuff_mcmc2glob_batches(varargin)
...
data.dsid = dsid;
data.X =  [T dsid];
% data_batch.dsid = dsid_batch;

% data_batch.X =  [T_batch dsid_batch];
data_batch = {};
...
% model.ssfun = @(params, data) sum( (data_batch.ydata(:,2:end)- mdl(data_batch.X(:,1), params) ).^2 );
...
if lsq
    ...
    results = [];
    [results,~,~,~]=mcmcrun(model,data_batch,params,options,results);
    [results,~,~,~]=mcmcrun(model,data_batch,params,options,results);
    [results,chain,s2chain,~]=mcmcrun(model,data_batch,params,options,results);
end
